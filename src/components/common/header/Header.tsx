      ...
      {isMenuOpen && (
        <div className="absolute top-24 right-8 bg-white shadow-lg rounded-lg p-4 w-48">
          <ul className="space-y-2">
            <li>
              <button onClick={() => console.log('Abriendo configuraciones...')} className="text-black">Configuración</button>
            </li>
            <li>
              <button onClick={handleLogout} disabled={isLoading}  className="text-red-600">  {isLoading  ? 'Cargando...' : 'Cerrar sesión'}</button>
            </li>
          </ul>
        </div>
      )}
