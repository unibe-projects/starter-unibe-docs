...
export const TableActivity = (doc: jsPDF, actividades: ActivityReport[], startY: number) => {
  ...
  actividades.forEach((actividad) => {
    ...
    doc.text(
      String(actividad.name || DEFAULT_TEXT),
      xOffsets[ZERO] + CELL_WIDTH / TWO,
      startY + ROW_HEIGHT / TWO,
      { align: TEXT_ALIGN },
    );
    doc.text(
      String(actividad.createdAt || DEFAULT_TEXT),
      xOffsets[ONE] + CELL_WIDTH / TWO,
      startY + ROW_HEIGHT / TWO,
      { align: TEXT_ALIGN },
    );
    doc.text(
      String(actividad.number_participants || DEFAULT_TEXT),
      xOffsets[TWO] + CELL_WIDTH / TWO,
      startY + ROW_HEIGHT / TWO,
      { align: TEXT_ALIGN },
    );
    doc.text(
      String(actividad.budget_used || DEFAULT_TEXT),
      xOffsets[THREE] + CELL_WIDTH / TWO,
      startY + ROW_HEIGHT / TWO,
      { align: TEXT_ALIGN },
    );
